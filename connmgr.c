...
void connmgr_server(int PORT, sbuffer_t* sbuffer, int fd){
    ...
    printf("Connmgr is shutting down.\n");
    sensor_data_t* sensor_data = malloc(sizeof(sensor_data_t));
    sensor_data->id = 0;
    sensor_data->ts = 0;
    sensor_data->value = 0;
    sensor_data->data_flag = false;
    sensor_data->storage_flag = false;
...
void *func(void *param){
    ...
    do {
            ...
            if(time_diff > TIMEOUT){
                sprintf(conn_log_event, "Sensor node %"PRIu16" connection has closed due to TIMEOUT.\n", data.id);
